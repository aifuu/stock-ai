...
KEYS=["status","updated_at","up_threshold","min_score_for_buy","nikkei_filter","atr_tp_multiplier","atr_sl_multiplier","hold_days","validation_signals","validation_win_rate","validation_avg_return","validation_pf","validation_dd","oos_signals","oos_win_rate","oos_avg_return","oos_pf","oos_dd","oos_validation_pf_ratio","mc_sizing","mc_10y_probability","mc_15y_probability","mc_20y_probability","mc_bankruptcy_probability","mc_p90_max_dd","strategy_name","selection_mode","regime_expectancy_json","source"]
def sign(p,secret):
...
b=passed.iloc[0]
policy={"status":"APPROVED","updated_at":datetime.now().isoformat(timespec="seconds"),"up_threshold":int(b.up_threshold),"min_score_for_buy":int(b.score_threshold),"nikkei_filter":bool(b.nikkei_filter),"atr_tp_multiplier":float(b.tp_multiplier),"atr_sl_multiplier":float(b.sl_multiplier),"hold_days":int(b.hold_days),"validation_signals":int(b.validation_signals),"validation_win_rate":float(b.validation_win_rate),"validation_avg_return":float(b.validation_avg_return),"validation_pf":float(b.validation_pf),"validation_dd":float(b.validation_dd),"oos_signals":int(b.oos_signals),"oos_win_rate":float(b.oos_win_rate),"oos_avg_return":float(b.oos_avg_return),"oos_pf":float(b.oos_pf),"oos_dd":float(b.oos_dd),"oos_validation_pf_ratio":float(b.oos_validation_pf_ratio),"mc_sizing":None,"mc_10y_probability":None,"mc_15y_probability":None,"mc_20y_probability":None,"mc_bankruptcy_probability":None,"mc_p90_max_dd":None,"strategy_name":str(b.strategy),"selection_mode":"REGIME_EXPECTED_RETURN","regime_expectancy_json":str(b.get("regime_expectancy_json","")),"source":"adversarial_strategy_validator","multi_oos_folds":int(b.folds),"positive_oos_folds":int(b.positive_oos_folds),"oos_compound_return":float(b.oos_compound_return),"oos_monthly_positive_ratio":float(b.oos_monthly_positive_ratio_mean)}
