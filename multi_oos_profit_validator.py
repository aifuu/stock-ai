...
def select(df, up, score, nikkei):
    ...
    if x.empty:
        return x
    # 同一日のTOP10も確率加重期待利益を優先。価格とTP/SLから事前期待値を計算する。
    x["profit_ev"] = (x.up_prob / 100.0) * ((x.take_profit - x.price) / x.price * 100.0) - (x.down_prob / 100.0) * ((x.price - x.stop_loss) / x.price * 100.0)
...
def evaluate(df, prices, params):
    ...
    for _, r in x.iterrows():
        ...
        if p is None:
            continue
        atr_ratio = max(0.01, min(20.0, ((float(r.take_profit) / float(r.price) - 1) / 3.0) * 100))
        ret = trade(p, r.date, float(r.price), atr_ratio, tp, sl, hold)
        if ret is not None:
            rows.append((r.date, float(ret)))
    if not rows:
        return {"signals": 0, "avg_return": 0.0, "pf": 0.0, "dd": 0.0, "monthly_positive_ratio": 0.0, "compound_return": 0.0}
    ...
    return {"signals": len(z), "avg_return": float(z.return.mean()), "pf": float(pf), "dd": float((eq / eq.cummax() - 1).min() * 100), "monthly_positive_ratio": float((monthly > 0).mean() * 100) if len(monthly) else 0.0, "compound_return": float((eq.iloc[-1] - 1) * 100), "final_capital": float(INITIAL_CAPITAL * eq.iloc[-1])}


def passes(s):
    return s["signals"] >= MIN_TRADES and s["pf"] >= MIN_PF and s["avg_return"] > 0 and s["dd"] >= -MAX_DD and s["compound_return"] > 0
...
print("=" * 80)
print("🛡️ FOUR-FOLD MULTI-OOS PROFIT GATE")
print(f"Folds={FOLDS} / 各OOS={FOLD_OOS_DAYS}営業日 / 合計OOS={FOLD_OOS_DAYS*FOLDS}")
print("100銘柄ユニバースの候補CSVをそのまま使用。コード内で銘柄を改変しません。")
print("=" * 80)
prices = {}
...
for fold in range(1, FOLDS + 1):
    ...
    dev_df, val_df, oos_df = c[c.date.isin(dev)], c[c.date.isin(val)], c[c.date.isin(oos_dates)]
    ...
    for params in PARAMS:
        ...
        if s["signals"] >= 20 and s["avg_return"] > 0 and s["pf"] >= 1:
            obj = 0.35*s["monthly_positive_ratio"] + 0.30*np.clip(s["compound_return"], -100, 500) + 0.20*np.clip(s["pf"], 0, 5)*10 + 0.15*np.clip(s["avg_return"], -5, 5)*10
    ...
    chosen = valid[:50]
    if not chosen:
        # 診断用にDEV上位を残すが、OOS採用はしない。
        chosen = [(0.0, p, evaluate(val_df, prices, p)) for _, p in validation_candidates[:50]]
    for _, params, vs in chosen:
        os = evaluate(oos_df, prices, params)
        name = f"UP{params[0]}_SCORE{params[1]}_NIKKEI{'ON' if params[2] else 'OFF'}_TP{params[3]}_SL{params[4]}_H{params[5]}"
        strategy_fold_rows.append({"fold": fold, "strategy": name, "up": params[0], "score": params[1], "nikkei": params[2], "tp": params[3], "sl": params[4], "hold": params[5], "validation_signals": vs["signals"], "validation_avg_return": vs["avg_return"], "validation_pf": vs["pf"], "validation_dd": vs["dd"], **{"oos_"+k:v for k,v in os.items()}, "oos_pass": passes(os)})

fold_table = pd.DataFrame(strategy_fold_rows)
fold_table.to_csv(FOLD_FILE, index=False, encoding="utf-8-sig")

# 同一パラメータを4 OOSで再確認し、少なくとも3/4 OOSが利益プラス、かつ全体もプラスのものだけ最終候補にする。
if fold_table.empty:
    final = pd.DataFrame()
else:
    grouped = []
    for name, g in fold_table.groupby("strategy"):
        ...
        pf_values = pd.to_numeric(g.oos_pf, errors="coerce").replace([np.inf], np.nan).dropna()
        total_signals = int(g.oos_signals.sum())
        daily_like = pd.to_numeric(g.oos_avg_return, errors="coerce")
        compound = float(((1 + daily_like.fillna(0) / 100).prod() - 1) * 100)
        avg_monthly = float(pd.to_numeric(g.oos_monthly_positive_ratio, errors="coerce").mean())
        worst_dd = float(pd.to_numeric(g.oos_dd, errors="coerce").min())
        grouped.append({"strategy": name, "folds": len(g), "positive_oos_folds": positive, "oos_signals": total_signals, "oos_pf_min": float(pf_values.min()) if len(pf_values) else 0.0, "oos_pf_mean": float(pf_values.mean()) if len(pf_values) else 0.0, "oos_avg_return_mean": float(daily_like.mean()), "oos_monthly_positive_ratio_mean": avg_monthly, "oos_worst_dd": worst_dd, "oos_compound_return": compound, "up": int(g.up.iloc[-1]), "score": int(g.score.iloc[-1]), "nikkei": bool(g.nikkei.iloc[-1]), "tp": float(g.tp.iloc[-1]), "sl": float(g.sl.iloc[-1]), "hold": int(g.hold.iloc[-1]), "oos_pass": positive >= MIN_POSITIVE_FOLDS and total_signals >= MIN_TOTAL_TRADES and compound > 0 and avg_monthly >= MIN_MONTHLY and worst_dd >= -MAX_DD})
    final = pd.DataFrame(grouped)
    final = final[final.oos_pass].copy()

if not final.empty:
    final["profit_objective"] = 0.45*final.oos_monthly_positive_ratio_mean + 0.35*np.clip(final.oos_compound_return, -100, 1000) + 0.20*np.clip(final.oos_pf_mean, 0, 8)*10
    final = final.sort_values(["profit_objective", "positive_oos_folds", "oos_compound_return"], ascending=False).reset_index(drop=True)
    ...
    final["oos_validation_pf_ratio"] = final.oos_pf_mean / final.oos_pf_mean.replace(0, np.nan)
    ...
    final = pd.DataFrame(columns=["final_status", "up_threshold", "score_threshold", "nikkei_filter", "tp_multiplier", "sl_multiplier", "hold_days"])

final.to_csv(FINAL_FILE, index=False, encoding="utf-8-sig")
fold_table.to_csv(OOS_FILE, index=False, encoding="utf-8-sig")
print("=" * 80)
print(f"4-FOLD結果: 最終PASS={len(final)} / {FOLDS} OOSのうち最低{MIN_POSITIVE_FOLDS}本が利益プラス")
if not final.empty:
    print("採用候補:", final.iloc[0].strategy)
else:
    print("採用候補なし → policyはPENDINGのまま。手動でAPPROVEDへ改ざんしない。")
