...
# 注: oos_validation_pf_ratioの閾値チェックはここでは行わない。
# adversarial_strategy_validator.py が各Fold単独のoos_pass判定で既に
# 外れ値免除ロジック込みで検証済みであり(multi_oos_profit_gate.pyのaggregate()も
# 同じ理由でここへの入力段階では再チェックしない)、この時点で
# adversarial_final_candidates.csvに載っている候補は既に通過済みの候補。
approved = approved[
