...
def _market_regime():
    try:
        ...
        kairi = float(last["kairi25"]); ret5 = float(last["ret5"])
...
def profit_priority(candidates):
    ...
    for c in candidates:
        ...
        price = float(c.get("price", 0) or 0); tp = float(c.get("tp", 0) or 0); sl = float(c.get("sl", 0) or 0)
        up = float(c.get("up_probability", 0) or 0) / 100.0; down = float(c.get("down_probability", 0) or 0) / 100.0
        if price <= 0: ev = -999.0
        elif direction == "SHORT":
            reward = max(0.0, (1.0 - tp / price) * 100.0); risk = max(0.0, (sl / price - 1.0) * 100.0)
            ev = down * reward - (1.0 - down) * risk
        else:
            reward = max(0.0, (tp / price - 1.0) * 100.0); risk = max(0.0, (1.0 - sl / price) * 100.0)
        ...
        item = dict(c); item["market_regime"] = regime; item["regime_preferred"] = bool(preferred); item["regime_bonus"] = regime_bonus
        item["profit_ev_pct"] = round(ev, 4); item["profit_priority"] = round(rank, 4); ranked.append(item)
...
def _policy_for_level(base_policy, spec):
    p = dict(base_policy); p["up_threshold"] = float(spec["up_threshold"]); p["min_score_for_buy"] = float(spec["min_score"]); p["nikkei_filter"] = False
    return p


def _passes_level(candidate, spec):
    direction = str(candidate.get("direction", "BUY")).upper(); up = float(candidate.get("up_probability", 0) or 0); down = float(candidate.get("down_probability", 0) or 0); flat = float(candidate.get("flat_probability", 0) or 0); score = float(candidate.get("score", 0) or 0)
    threshold = float(spec["up_threshold"]); score_min = float(spec["min_score"])
...
def _print_gate_diagnostics(emergency_pool, scanned):
    ...
    print("\n" + "=" * 86); print("🔎 PAPER候補ゲート診断（ペーパー専用。OOS/Adversarialとは独立）"); print("=" * 86)
    print(f"スキャン成功: {int(scanned or 0)}"); print("固定条件通過: BUY=UP>DOWN / SHORT=DOWN>UP かつ Flat<50%"); print(f"固定条件通過件数: {len(pool)}\n")
...
def scan_candidates_progressive(policy):
    """Return the same two-value contract as profit_top10_paper.scan_candidates.
    selection_level is stored on each selected candidate, not as a third return value.
    """
    last_scanned = 0
    for spec in PAPER_ENTRY_LEVELS:
        raw, scanned = _original_scan(_policy_for_level(policy, spec)); last_scanned = max(last_scanned, int(scanned or 0)); pool = raw or []
        qualified = [c for c in pool if _passes_level(c, spec)]; ranked = profit_priority(qualified); top10 = ranked[:TOP10]
        ...
        if top10:
            ...
            for rank, c in enumerate(top10, 1):
                ...
                print(f"  {rank:02d}. {c.get('direction', 'BUY')} {c['ticker']} score={c['score']:.1f} UP={c['up_probability']:.1f}% DOWN={c.get('down_probability', 0):.1f}% REGIME={'PREFERRED' if c.get('regime_preferred') else 'OTHER'} EV={c.get('profit_ev_pct', 0):+.2f}% rank={c.get('profit_priority', 0):.1f}")
...
def close_positions_with_cooldown(state, policy, now):
    before = {str(p.get("ticker")) for p in state.get("positions", []) if p.get("ticker")}; messages = _original_close(state, policy, now); after = {str(p.get("ticker")) for p in state.get("positions", []) if p.get("ticker")}
...
def open_top1_only(state, policy, candidates, today):
    ...
    if opened:
        p = state["positions"][-1]; p["allocation"] = 1.0; p["selection_mode"] = top1.get("selection_mode", "normal"); p["selection_level"] = int(top1.get("selection_level", 1)); p["top10_rank"] = int(top1.get("top10_rank", 1)); p["market_regime"] = top1.get("market_regime", "neutral"); p["regime_preferred"] = bool(top1.get("regime_preferred", False)); p["profit_ev_pct"] = float(top1.get("profit_ev_pct", 0.0)); p["profit_priority"] = float(top1.get("profit_priority", 0.0))
        print(f"🏆 TOP→TOP1 ENTRY: {top1.get('direction', 'BUY')} {top1['ticker']} LEVEL={p['selection_level']} MODE={p['selection_mode']} REGIME={p['market_regime']} PREFERRED={p['regime_preferred']} score={top1['score']:.1f} UP={top1['up_probability']:.1f}% DOWN={top1.get('down_probability', 0):.1f}%")
    return opened
